...
async def on_message(message):
    ...
    if message.content.startswith(">>"):
        ...
        if content == "한줄소개설문지":
            await message.channel.send("https://forms.gle/qw6SgwN6rrGmwqiT7")
            return
        ...
        if content == "맵추천":
            # 파리 호라이즌
            maps = "네팔 리장타워 부산 오아시스 일리오스 볼스카야인더스터리 아누비스신전 하나무라 66번국도 감시기지:지브롤터 도라도 리알토 쓰레기촌 하바나 눔바니 블리자드월드 아이헨발데 왕의길 할리우드 뉴퀸스트리트 서킷로얄 미드타운 콜로세오"
